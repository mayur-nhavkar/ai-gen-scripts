...
export default function () {
    ...
    // Create Cart
    res = http.post('http://sample_app:8002/api/v1/cart', JSON.stringify({}), { tags: { endpoint: '/api/v1/cart' }, headers: { 'Content-Type': 'application/json' } });
    check(res, { 'status is 2xx': (r) => r.status >= 200 && r.status < 300 });
    sleep(1);
    ...
    // Checkout
    res = http.post('http://sample_app:8002/api/v1/checkout', JSON.stringify({}), { tags: { endpoint: '/api/v1/checkout' }, headers: { 'Content-Type': 'application/json' } });
    check(res, { 'status is 2xx': (r) => r.status >= 200 && r.status < 300 });
    sleep(1);
}