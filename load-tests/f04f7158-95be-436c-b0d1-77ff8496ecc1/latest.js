...
export default function () {
    ...
    res = http.post('http://sample_app:8002/api/v1/cart', cartPayload, { ...params, headers: { 'Content-Type': 'application/json' } });
    check(res, { 'status is 2xx': (r) => r.status >= 200 && r.status < 300 });
    sleep(1);
    ...
    res = http.post('http://sample_app:8002/api/v1/checkout', checkoutPayload, { ...params, headers: { 'Content-Type': 'application/json' } });
    check(res, { 'status is 2xx': (r) => r.status >= 200 && r.status < 300 });
    sleep(1);
